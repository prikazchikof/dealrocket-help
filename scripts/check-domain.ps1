...
foreach ($path in $deepLinks.Keys) {
	try {
		...
		if ($path -eq '/start/data-quality/' -and $article.Content -notmatch 'href="https://dealrocket\.ru/app/contacts"') {
			$errors.Add("$path does not link to the DealRocket contacts page")
		}
